...
from sqlalchemy import Column, String, Boolean, Integer, ForeignKey, Table, DateTime, DECIMAL, UniqueConstraint
...
class User(Base):
    ...
    obj_guid = Column(String(16), comment="系统生成，不可重复，证书", unique=True,nullable=False)
    nick_name = Column(String(64), comment="昵称", nullable=False)
    ...
    level_id = Column(Integer, comment="会员等级？", default=0)
...
class UserLog(Base):
    ...
    operate_status = Column(Integer, comment="操作类型：(0密码修改，1资料编辑，2登陆，3注册)", default=0)
